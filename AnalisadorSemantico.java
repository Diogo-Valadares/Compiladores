...
public class AnalisadorSemantico {
	...
	public String inserirSimbolo(Object variavel, TipoVariavel tipo) {
        if(identificadores.contains(variavel)){
            System.out.println("\n\nErro  na linha "+(currToken.left+1)+": Variável \"" + variavel + "\" já foi previamente declarada");
    ...
    public Identificador obterIdentificador(Object variavel) {
		...
		if (identificador == null) {
			System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": Variável \"" + variavel + "\" precisa ser declarada antes de ser usada."));
			return null;
		} 
	...
	public String atribuirVariavel(Object variavel, Expressao valor) {
		...
		if(valor.ehVariavel){
            if(!identificadores.containsKey(valor.resultado)){
                System.out.println("\n\nErro na linha "+(currToken.left+1)+": Variável \"" + valor.resultado + "\" precisa ser declarada antes de ser usada.");
                return null;
		    }            
            if(!identificadores.get(valor.resultado).estaAtribuido){
                System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": Variável \"" + valor.resultado + "\" precisa ser atribuida antes de ser usada."));
                return null;
            }
        ...
        if(tipo1 != tipo2 && tipo1 != TipoVariavel.INT && tipo1 != TipoVariavel.FLOAT && tipo2 != TipoVariavel.INT && tipo2 != TipoVariavel.FLOAT){
            switch(tipo1){  
                case BOOL:
                    System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": O tipo boolean da variável \"" + variavel + "\" só pode receber \"true\" ou \"false\" na atribuição"));
                    break;
                case INT:
                    System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": O tipo inteiro da variável \"" + variavel + "\" só pode receber números inteiros na atribuição"));
                    break;
                case FLOAT:
                    System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": O tipo float da variável \"" + variavel + "\" só pode receber números decimais na atribuição"));
                    break;
                case CHAR:
                    System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": O tipo char da variável \"" + variavel + "\" só pode receber um caractere por vez"));
                    break;
            } 
        }        
	...
	public Expressao testarExpressao(Expressao var1, Expressao var2, String operador) {
        ...
        if(var1.ehVariavel){
            if(!identificadores.containsKey(var1.resultado)) {
                System.out.println("\n\nErro na linha "+(currToken.left+1)+": Variável \"" + var1.resultado + "\" precisa ser declarada antes de ser usada.");
                return null;
		    }            
            if(!identificadores.get(var1.resultado).estaAtribuido){
                System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": Variável \"" + var1.resultado + "\" precisa ser atribuida antes de ser usada."));
                return null;
            }
		...
		if(var2.ehVariavel){
            if(!identificadores.containsKey(var2.resultado)){
                System.out.println("\n\nErro na linha "+(currToken.left+1)+": Variável \"" + var2.resultado + "\" precisa ser declarada antes de ser usada.");
                return null;
		    }            
            if(!identificadores.get(var2.resultado).estaAtribuido){
                System.out.println(String.format("\n\nErro na linha "+(currToken.left+1)+": Variável \"" + var2.resultado + "\" precisa ser atribuida antes de ser usada."));
                return null;
            }
        ...
        switch(operador){
            ...
            case "/":
                if(tipo1 == TipoVariavel.CHAR || tipo2 == TipoVariavel.CHAR){
                    System.out.println("\n\nErro na linha "+(currToken.left+1)+": Operadores aritméticos não podem ser executados com o tipo char");
                    return null;
                }
                if(tipo1 == TipoVariavel.BOOL || tipo2 == TipoVariavel.BOOL){
                    System.out.println("\n\nErro na linha "+(currToken.left+1)+": Operadores aritméticos não podem ser executados com o tipo bool");
                    return null;
                }
            ...
            case "!=":
                if(tipo1 != tipo2){
                    System.out.println("\n\nErro na linha "+(currToken.left+1)+": É apenas possivel comparar variáveis de mesmo tipo(" + tipo1 + " != " + tipo2 +")");
                    return null;
                }
                break;
            case "&&":
            ...
            case "^":
                if(tipo1 != TipoVariavel.BOOL || tipo2 != TipoVariavel.BOOL){
                    System.out.println("\n\nErro na linha "+(currToken.left+1)+": operadores lógicos só podem ser realizados entre tipos booleanos");
                    return null;
                }
                break;
        }
